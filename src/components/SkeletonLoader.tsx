  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-50/90 border border-purple-200/80 text-[#6055ea] text-xs font-medium shadow-2xs">
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
            className="w-3.5 h-3.5 flex items-center justify-center"
          >
            <Sparkles className="w-3.5 h-3.5 text-[#6055ea]" />
          </motion.div>
          ...
          <span className="text-[10px] text-purple-400 font-mono pl-1 border-l border-purple-200">
        ...
        <div className="flex items-center gap-1.5 text-xs text-zinc-400">
          ...
          <span className="text-zinc-400 hidden sm:inline">กำลังประมวลผลคำตอบและโครงสร้างโค้ด</span>
        </div>
      </div>

      {/* Shimmering Response Preview */}
      <div className="space-y-3 pt-1">
        <div className="h-4 bg-gradient-to-r from-zinc-200/90 via-purple-100/60 to-zinc-200/90 rounded-md w-[85%] animate-pulse" />
        <div className="h-4 bg-gradient-to-r from-zinc-200/80 via-zinc-100 to-zinc-200/80 rounded-md w-[95%] animate-pulse" />
        <div className="h-4 bg-gradient-to-r from-zinc-200/80 via-zinc-100 to-zinc-200/80 rounded-md w-[68%] animate-pulse" />
      </div>

      {/* Code Block Skeleton */}
      <div className="rounded-xl border border-zinc-200/80 bg-zinc-900/95 p-4 space-y-3 shadow-xs overflow-hidden relative">
      ...
      <div className="space-y-2 pt-1">
        <div className="h-4 bg-gradient-to-r from-zinc-200 via-zinc-100 to-zinc-200 rounded-md w-[78%] animate-pulse" />
        <div className="h-4 bg-gradient-to-r from-zinc-200 via-zinc-100 to-zinc-200 rounded-md w-[45%] animate-pulse" />
      </div>
    </motion.div>
  );
};
...
/**
 * FileSkeleton: Shimmer placeholder while a file is being read/parsed.
 */
export const FileSkeleton: React.FC<{ name?: string }> = ({ name }) => {
  return (
    <div className="flex items-center gap-2.5 px-3 py-2 bg-zinc-50 border border-zinc-200 rounded-xl animate-pulse min-w-[180px] max-w-xs shadow-2xs">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="h-3 bg-zinc-300 rounded w-[75%]" />
        <div className="h-2 bg-zinc-200 rounded w-[45%]" />
      </div>
    </div>
  );
};
